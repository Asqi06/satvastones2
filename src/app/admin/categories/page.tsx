    ...
    <div className="space-y-12">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-8 border-b border-luxury-brown/5 pb-10">
        <div>
          ...
          <h1 className="text-4xl lg:text-6xl font-serif text-luxury-brown">Categories</h1>
        </div>
        <button
      ...
      {showForm && (
        <div className="bg-white border border-luxury-brown/5 p-10 animate-luxury-fade relative overflow-hidden shadow-2xl">
          ...
          <h2 className="text-xl font-serif text-luxury-brown mb-8 tracking-widest uppercase">Category Specification</h2>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-8 relative z-10">
            <div className="space-y-2">
              <label className="text-[9px] text-luxury-brown/30 tracking-[0.3em] font-bold uppercase">Category Name</label>
              <input
                type="text"
                ...
                required
                className="w-full bg-luxury-cream/20 border-b border-luxury-brown/10 px-4 py-4 text-luxury-brown text-xs tracking-widest transition-all focus:outline-none focus:border-luxury-gold placeholder-luxury-brown/30"
              />
            </div>
            <div className="space-y-2">
              <label className="text-[9px] text-luxury-brown/30 tracking-[0.3em] font-bold uppercase">Parent Category</label>
              <select
                value={form.parentId}
              ...
              >
                <option value="" className="bg-white text-luxury-brown">Top Level Category</option>
                {categories.map((c) => (
            ...
            <div className="sm:col-span-2 space-y-2">
              <label className="text-[9px] text-luxury-brown/30 tracking-[0.3em] font-bold uppercase">Description</label>
              <textarea
                placeholder="SPECIFY THE ESSENCE OF THIS COLLECTION"
                value={form.description}
                onChange={(e) => setForm({ ...form, description: e.target.value })}
                className="w-full bg-luxury-cream/20 border-b border-luxury-brown/10 px-4 py-4 text-luxury-brown text-xs tracking-widest transition-all focus:outline-none focus:border-luxury-gold h-20 resize-none placeholder-luxury-brown/30"
              />
            </div>
      ...
      <div className="bg-white border border-luxury-brown/5 overflow-hidden shadow-sm">
        <table className="w-full">
          <thead>
            <tr className="border-b border-luxury-brown/5 bg-luxury-cream/30">
              <th className="text-left text-[9px] text-luxury-brown/30 uppercase tracking-[0.3em] px-8 py-6 font-bold">Name</th>
              <th className="text-left text-[9px] text-luxury-brown/30 uppercase tracking-[0.3em] px-8 py-6 font-bold">Slug</th>
              <th className="text-left text-[9px] text-luxury-brown/30 uppercase tracking-[0.3em] px-8 py-6 font-bold">Description</th>
          ...
          <tbody className="divide-y divide-luxury-brown/5">
            {categories.map((cat) => (
              <tr key={cat.id} className="hover:bg-luxury-cream/20 transition-colors group">
                ...
                <td className="px-8 py-6">
                  <p className="text-luxury-brown/40 text-[11px] font-light italic truncate max-w-xs">{cat.description || "No description"}</p>
                </td>
        ...
        {categories.length === 0 && (
          <div className="py-32 text-center">
            <p className="text-luxury-brown/10 text-xs tracking-[0.5em] uppercase font-bold">No categories found</p>
          </div>
        )}
      </div>
    </div>
