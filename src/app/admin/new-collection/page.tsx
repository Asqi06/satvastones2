...
export default function NewCollectionPage() {
  ...
  const toggleNewCollection = async (productId: string, currentStatus: boolean) => {
    setToggling(productId);
    try {
      await fetch(`/api/products/${productId}/new-collection`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ isNewCollection: !currentStatus }),
      });
      setProducts((prev) =>
        prev.map((p) =>
          p.id === productId ? { ...p, isNewCollection: !currentStatus } : p
        )
      );
    } catch {
      console.error("Failed to toggle new collection status");
    } finally {
      setToggling(null);
    }
  };
  ...
  const newCollectionCount = products.filter((p) => p.isNewCollection).length;
    ...
    <div className="space-y-12">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-8 border-b border-luxury-brown/5 pb-10">
        <div>
          ...
          <h1 className="text-4xl lg:text-6xl font-serif text-luxury-brown">New Collection</h1>
          <p className="text-luxury-brown/40 text-[10px] tracking-[0.3em] uppercase font-bold mt-4">
            {newCollectionCount} products featured
          </p>
        </div>
        ...
        {filteredProducts.map((product) => (
          ...
          >
            ...
            <div className="p-6">
              ...
              <button
                onClick={() => toggleNewCollection(product.id, product.isNewCollection)}
