...
import { Plus, Edit, Trash2, GripVertical, Eye, EyeOff, Check } from "lucide-react";
...
export default function HomepageSectionsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await fetch(`/api/homepage-sections/${editingId}`, {
          method: "PUT",
      ...
      setShowForm(false);
      setEditingId(null);
      setForm({ title: "", sortOrder: "0", isActive: true, productIds: [] });
    } catch {
    ...
    <div className="space-y-12">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-8 border-b border-luxury-brown/5 pb-10">
        <div>
          ...
          <h1 className="text-4xl lg:text-6xl font-serif text-luxury-brown">Sections</h1>
        </div>
        <button
          onClick={() => {
          ...
          <form onSubmit={handleSubmit} className="relative z-10">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 mb-8">
              <div className="space-y-2">
                <label className="text-[9px] text-luxury-brown/30 tracking-[0.3em] font-bold uppercase">Section Title</label>
                <input
                  type="text"
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-luxury-brown/5">
              {sections.map((section) => (
                <tr key={section.id} className="hover:bg-luxury-cream/20 transition-colors group">
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-2 text-luxury-brown/30">
                      <GripVertical className="w-4 h-4" />
                      <span className="text-sm font-bold">{section.sortOrder}</span>
                    </div>
                  </td>
                  <td className="px-8 py-6">
          ...
          {sections.length === 0 && !loading && (
            <div className="py-32 text-center">
              <p className="text-luxury-brown/10 text-xs tracking-[0.5em] uppercase font-bold">No sections configured</p>
            </div>
          )}
        </div>
      </div>
